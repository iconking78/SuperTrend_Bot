...
// ─── Track last known direction to detect flips ───────────────────────────────
let lastDirection = null;  // null = not yet initialized
let isFirstRun    = true;  // skip trading on first check
...
async function checkSignal() {
  try {
    ...
    // First run — just record direction, never trade
    if (isFirstRun) {
      lastDirection = curr;
      isFirstRun    = false;
      console.log(`Startup: recording initial direction (${curr}). Watching for flips...`);
      return;
    }

    // Only fire if direction actually flipped since last check
